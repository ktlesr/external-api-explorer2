...
import { Save, RotateCcw, Sparkles, Database, Settings2, FileText, Key, Cloud, Loader2, CheckCircle2 } from "lucide-react"
...
interface ApiKeys {
  ...
  internalApiKey: string // Bu artık veritabanında da tutulacak
}
...
export default function AdminPage() {
  ...
  // --- 1. BAŞLANGIÇ: LocalStorage'dan Bağlantı Bilgilerini Al ---
  useEffect(() => {
    const init = async () => {
      ...
      if (savedApiKeys) {
        ...
        // State'i güncelle (Supabase URL ve Anon Key buraya gelir)
        setApiKeys(prev => ({
            ...prev,
            supabaseUrl: keys.supabaseUrl || "",
            supabaseAnonKey: keys.supabaseAnonKey || "",
            // internalApiKey'i henüz set etmiyoruz, onu DB'den çekeceğiz
        }))
  ...
  const fetchLatestConfig = async (url: string, key: string) => {
    ...
    try {
      ...
      if (data) {
        ...
        // 2. Internal API Key'i Yükle (Eğer varsa)
  ...
  // --- AYARLARI SUPABASE'E KAYDET (INSERT) ---
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      const dbPayload = {
        ...
        internal_api_key: apiKeys.internalApiKey, // ARTIK BU DA KAYDEDİLİYOR ✅
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <Button onClick={handleSaveConnection} variant="secondary" className="w-full">
                        {connectionStatus === 'success' ? <CheckCircle2 className="mr-2 size-4 text-green-500" /> : null}
                        Bağlantıyı Güncelle
                    </Button>
                </CardContent>
              </Card>
